...
> **Actualización (Fase 2).** El diagnóstico de arriba se sostiene, pero la
> solución no fue una interfaz más ancha: con la cache en el medio resultaron ser
> dos preguntas distintas, resueltas con dos tipos de dato (`TagSample` y
> `TagState`). Ver decisión 7.
...
- Una versión anterior de este archivo justificaba `Device` afirmando que
  ...
  atribuible al origen de lectura. Ver *Anomalía observada* al final del archivo.

## Anomalía observada: `SourceTimestamp` atrasado ~7 minutos
